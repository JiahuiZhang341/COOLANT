...
import process_data_cfnd as process_data
import copy
...
class Rumor_Data(Dataset):
    def __init__(self, dataset):
        ...
        print('TEXT: %d, label: %d'
               % (len(self.text), len(self.label)))
...
class FakeNewsDataset(Dataset):
    def __init__(self,input_three,image,label) :
    ...
    def __getitem__(self,idx):
        return self.input_three[0][idx],self.input_three[1][idx],self.input_three[2][idx],self.image[idx],self.label[idx]
...
def main(args):
    ...
    for i, (train_text, train_image, train_labels) in enumerate(tqdm(train_loader)):
    ...
    dataset = FakeNewsDataset(input_three,all_images,all_train_labels)
    ...
    all_token_type_ids = []
    print('building model')
    for i, (train_text, train_image, train_labels) in enumerate(tqdm(test_loader)):
            train_text, train_image, train_labels= \
                train_text, to_var(train_image), \
                to_var(train_labels)
            #text_features = save_features_batch_by_batch(args, train_text)
