...
def evaluate(clip_module, multi_model,vali_dataloader,device):
    ...
    val_true, val_pred = [], []
    with torch.no_grad():
        for index,(batch_text0,batch_text1,batch_text2,batch_image,batch_label) in enumerate(vali_dataloader):
...
def train_val_test():
    ...
    optimizer = AdamW(bert_multi_model.parameters(), lr=5e-6, weight_decay=1e-4) 
    ...
    for i in range(config.epochs):
        ...
        for index,(batch_text0,batch_text1,batch_text2,batch_image,batch_label) in enumerate(train_loader):
